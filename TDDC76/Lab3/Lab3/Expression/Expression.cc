 ...
 * konstruktorer
*/
Expression::Expression(Expression_Tree* node){
  if(node  == nullptr) 
    {
      tree = nullptr ;
    }
...
//Kopiatorkonstruktor
Expression::Expression(const Expression& node)
{
...
//operatorer
void Expression::operator=(const Expression& node)
{
  if(node.tree == nullptr)
    {
      delete tree;
...
void Expression::operator=(Expression&& node)
{
  delete tree;
  tree = node.tree;
  node.tree = nullptr;
...
long double Expression::evaluate() const
{
  if(tree == nullptr)
    {
      throw Expression_Error(" Tomt uttryck för evaluate \n");
    }
  else
    {
...
std::string Expression::get_postfix() const
{
  if(tree == nullptr)
    {
      throw Expression_Error(" Tomt uttryck för get_postfix \n");
    }
  else
    {
 ...
 * print_tree()
 */
void Expression::print_tree(ostream& out) const
{
  if(tree == nullptr)
    {
      throw Expression_Error("Finns inget träd att rita ut.");
    }
   tree->print(out, 1);
}
 ...
 * swap(other)
 */
void Expression::swap(Expression& temp)
{
  Expression_Tree* temporary = tree;
  tree = temp.tree;
  temp.tree = temporary;
